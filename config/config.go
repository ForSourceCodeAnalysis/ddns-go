...
// DnsConfig 配置
// 这个配置就是用户在本服务web页面配置的
// DNS服务商及Ipv4,Ipv6三部分配置
...
// DNS DNS配置
// 准确说应该是 dns open api 信息，就像百度网盘开放api一样
type DNS struct {
	// 名称。如：alidns,webhook
	Name   string //DNS 服务商名称
	ID     string //AppkeyId,SecretId 每个服务商定义不同，有的可能没有比如Cloudflare
	Secret string //AppSecret,token 每个服务商定义不同，但都是同样的意思，请求服务商时需要提供的认证密钥
}
